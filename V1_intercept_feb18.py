# -*- coding: utf-8 -*-
"""
Created on Thu Jul 23 11:50:17 2020

@author: rsalisbury

Version 2_18_spy: The purpose of this version is to address questions posed by
Dennis in an email on Oct. 13 2020:
    
    For either the demand structure of market structure settings, can you find an example in which a correct MM firm with beliefs about the rival based on self projection outperforms a passive correct MM firm that correctly understands its rival’s mental model but does not try to shape it through overproduction?  

    The idea is that selfprojection may shape the learning of the rival in a way similar to overproducing (trick) shapes the learning of the rival. A difference is that with overproduction, the sophisticated firm recoups its investment by ending overproduction after, say 20 periods.  For the example to exist, it would be the equivalent of overproducing for the entire 100 periods.  Perhaps that doesn’t work.   However, it is still interesting to ask, suppose one had self-projection for 20 periods, followed by passive optimization, would that be better than always being passive?  (This would be like a 20 period overproduction, followed by normal sophisticated optimization.)  I would liken this scenario as the firm was unsophisticated, then searched after 20 periods and learned about its rivals model and optimized accordingly.

Nothing substantive in the code should be changed, but I wanted to save this version separately so I don't mess up anything in the v2_17_spy version and so this can be run at the same time as that.
...
version_no = '1_int_'
##version 3_3: fixed the chooseQ function for the intercept model.
...
class Firm:
    ...
    PERIOD 0: The firm reflects on prices and quanitites it observed in the pre-history and calibrates based
    on its history to decide its initial betas. Since the history is deterministically derived from the true betas,
    ...
    def gen_init_history(self, init_hist_length, prehistmean, prehistsd,
        ...
        '''
        v2_6 changes:
        - Added firm_num to the parameters
        '''
        ...
        for i in range(hist_len):
            ...
            while j < len(X):
                x = X[j]
                while x < 0:
                    #print("redraw x in prehist")
                    #output_file.write("redraw x in prehist")
                    X = np.random.multivariate_normal(pre_hist_means, covar_matrix)
                    x = X[j]
                j+=1
            '''
            ...
            '''
            CHANGED IN V2_5: Used price function that is used in run_sim, instead of other price fcn.
            '''
            #p = np.inner(weights_mm, [1] + values)
            ##Assuming self.mm[0] = self.mm[3] = 1, then the following is equivalent:
            ##p = alpha + ({1 if F1, 0 if F2}*beta1 + {1 if F1, 0 if F2}*beta2 + beta3) * Q
        ...
        '''
        ####################################################
        # Price v Q Graphs for each iteration's prehistory #
        ####################################################
        
        #Comment out diagnostic graphs for high iterations to run faster.
        plot_destination = os.path.join(directory, 'Qhistplots')
        if not os.path.exists(plot_destination):
            os.makedirs(plot_destination)
            
        #for debugging (added v2_5): graph the pre-histories.
        prehist_df = pd.DataFrame(H)
        Q_df = pd.DataFrame(Q, index = [0]).T
        prehist_Q_df = pd.concat([prehist_df, Q_df], axis = 1, sort = True).reset_index()
        prehist_Q_df.columns = ['period', 'price', 'x1_Q', 'x2_Q', 'x3_Q', 'Q']
        #print(prehist_Q_df)
        #print(Q_df)
        prehist_fig = sns.regplot(x = 'Q', y = 'price', data = prehist_Q_df)
        prehist_fig.set(xlim = (0, 3.5), ylim = (20, 115))
        plt.savefig(os.path.join(directory, 'Qhistplots', 'Qhistplot' + str(filebegin) + '_iteration' + str(runsim_it) + '.png'), bbox_inches='tight')
        plt.close()
        plt.cla()
        plt.clf()

        #for debugging (added v2_6): record history in text, not just graph.
        prehist_file_name = str("%s" % runsim_it + "_firm" + "%s" % firm_num)
        prehist_path = os.path.join(directory, 'prehist_info')
        if not os.path.exists(prehist_path):
            os.makedirs(prehist_path)
        prehist_file = open(os.path.join(directory, 'prehist_info', prehist_file_name), 'w+')
        prehist_file.write(str(prehist_Q_df))
        prehist_file.close()

        #for debugging (added v2_6): record x shocks.
        prehist_xshocks = pd.DataFrame(X_shocks)
        xshock_file_name = str("%s.txt" % runsim_it + "_firm" + "%s" % firm_num)
        xshock_path = os.path.join(directory, 'xshock')
        if not os.path.exists(xshock_path):
            os.makedirs(xshock_path)
        xshock_file = open(os.path.join(directory, 'xshock', xshock_file_name), 'w+')
        xshock_file.write(str(prehist_xshocks))
        xshock_file.close()
        '''
    ...
    def chooseQ(self, periods, currentP, xvars, max_q, mean, sd, trick_params, market_type,
        ...
        ###INTERCEPT MODEL: CAN ONLY BE UPWARD SLOPING OR FLAT IF THE IND_BETA TERM IS###
        if (weights[3] * mm[3]) >= 0:
...
#From Anoop's earlier code: 1,.4,1,.4,6,1

##BEFORE RUNNING: CHECK PHX_Var and MAX_Q!##
...
#This version is not to be used for trick, so these are just to fill places:
...
'''
THIS SECTION ADDS PARAMETERS FOR NON-SWEEPING RUNS.
- dennis_notes: addressing issues dennis brought up in his oct. 27 note.
    - _1: comparing self-reflection to sophistication for the duopoly model.
    - _2: comparing self-reflection to sophistication for the monopoly model.
    - _3: failed attempts & trouble-shooting: comparing self-reflection to manipulation for the duopoly model.
    - _4: comparing self-reflection to manipulation for the duopoly model
    - _5: comparing for the monopoly model.
    - _6: fixed two_graph_overlap issue by adding a dfbeta for each graph so that the graphs of betas start in p.0
        - also fixed the labeling issue caused by having the wrong things in model_list
        - _2: changed how the idio beta is selected so that it is also selected from dfbeta.
            This version is what went into the response to Dennis.
- correct_incorrect_overlap: creating Figs 0 for the paper, Nov. 1.
    - _1: had both of the graphs taking the same initial weights, changed this so that they each take a different set of initial weights (as suitable to own model)
    - _2: ran with the correct initial weights for the model. (to test earlier changes); didn't work bc iw was a list, changed it to a dictionary.
    - _3: ran with the wrong models in model_list.
    - _4: made it greyscale; trying out changing the names to show up as "Both incorrect" instead of both_incorrect
        - Decided that the legend doesn't really matter bc I will probably edit it out and make a new one inside of the paper.
- _baseduopsr_forpub: creating Figs 1 for the paper, Nov. 1.
- _tgo: three graph overlap development
    - _1: first run of the graphing fcn three_graph_overlap, the purpose of which is to
          create a graph for each firm that shows its profits under self-reflection, 
          simple sophistication, and manipulation.
    - _2: fixed some indexing errors, changed the line colors
    - _3: changed annotation indexes.
    - _4: annotating firm 1's profit using zipped list
    - _5: annotating all firms for firm 1 and adding the firm 2 graph
    - _6: fixing annotations by making the face color none and moving the text; changed the face color back to white bc its more legible
    - _7: fixing labeling position to make them less overlapping.
- fig1_captesting: trying to figure out why the both_incorrect graph has irregular features 
(profit graph is too bumpy for number of iterations) by testing whether the model hits capacity for some reason.
    --> Note: the irregular-looking bumps in the profit graph are not present at low iterations (100 its, 1000 its)
    --> Note: weirdly, when I did another 10000 it run, there were no irregular bumps showing up.
    - _2: running at 10000 its again to make sure this issue is resolved.
- oldgraphrecreate: trying to figure out which parameter changes were made between this version and
    versions in the 2018 paper by changing parameters and using this code to create old graphs.
    ... fig1: refers to figure 1 sr_base_duop, sr, beta=-5, from the menon yao 2018 paper (since the current figure numbering has been changing)
        - _1: changed phx_var from 0.5 to 0.3; changed max_q from 50 to 5.
    ... fig3: refers to fig 3 in menon yao 2018 - sr_base, sr, beta = +5
        - _1: changing phx_var from 0.5 to 0.3; changed max_q from 50 to 5.
'''
...
'''
#CHANGE BETAS HERE, don't change code inside loop
#Packing up beta_option_list to sweep over, according to sweep type
#Note: inc_idio_beta indicates the incorrect firm's idiosyncratic beta.
if sweep_type == 'true':
    #This sweeps over the true idiosyncratic beta. Both firm 1 and firm 2 have the same idiosyncratic beta magnitude (the true beta), although they still have different mental models.
    beta_option_list = (-7.5, -5, -2.5, 0, 2.5)
    ind_beta, inc_ind_beta = -10, -10
elif sweep_type == 'incorrect':
    #This sweeps over the incorrect person's idiosyncratic beta without changing the true beta or the correct firm's betas
    beta_option_list = (-7.5, -5, -2.5, 0, 2.5)
    idio_beta, ind_beta = -5, -10
elif sweep_type == 'true_balanced':
    #This sweeps over the true idiosyncratic beta while 'balancing' it with the industry beta: the industry beta and idiosyncratic beta will always sum to -15.
    beta_option_list = ((-10,-5),(-7.5,-7.5),(-5,-10),(-2.5,-12.5),(0,-15),(2.5,-17.5),(5,-20))
elif sweep_type == 'incorrect_balanced':
    #This sweeps over the incorrect person's idiosyncratic beta while 'balancing' it with the industry beta.
    beta_option_list = ((-10,-5),(-7.5,-7.5),(-5,-10),(-2.5,-12.5),(0,-15),(2.5,-17.5),(5,-20))
    idio_beta, ind_beta = -5, -10
else:
    print('invalid sweep type')
...
for beta_option in beta_option_list:
    
    #Unpacking beta_option from the list to be added to param list, according to sweep type
    if sweep_type == 'true':
        idio_beta = beta_option
        inc_idio_beta = beta_option
        #ind_beta already set above
    elif sweep_type == 'incorrect':
        inc_idio_beta = beta_option
        inc_ind_beta = ind_beta #added in version 2_11; how did it work without this?
        #correct betas already set above
    elif sweep_type == 'true_balanced':
        idio_beta, ind_beta = beta_option
        inc_idio_beta, inc_ind_beta = beta_option
    elif sweep_type == 'incorrect_balanced':
        inc_idio_beta, inc_ind_beta = beta_option
        #correct betas already set above
    else:
    ...
    if 'sr_base_duop' in model_list:
        #BASE DUOP PARAM LIST, SELF-REFLECTIVE:
        param_list.append([2, 'q','sr', 'base_duop',[100,idio_beta,0,ind_beta],[[100,idio_beta,0,ind_beta],
                                                                               [100,0,inc_idio_beta,inc_ind_beta]],
    ...
    if 'so_base_duop' in model_list:
        #BASE DUOP PARAM LIST, SOPHISTICATED:
        #FIRM 1 IS SOPHISTICATED.
        param_list.append([2, 'q','so', 'base_duop',[100,idio_beta,0,ind_beta],[[100,idio_beta,0,ind_beta],
                                                                             [100,0,idio_beta,ind_beta]],
                          [[1,1,0,1],[1,0,1,1]],phlen,[1,phx_var,1,x_var,2,phq_var],
                          50,periods,periods,max_q,mult_cap,iterations,
                          trick_params, 1, 0, #trick_params, monop_firm, so_firm
                          'six_per_page', os.path.join(directory, 'cm=so_base_duop'),'base_so',2])

    if 'sr_monop_duop' in model_list:
        #MONOP_DUO PARAM LIST, SELF-REFLECTIVE:
        param_list.append([2, 'q','sr', 'monop_duop',[100,idio_beta,0,ind_beta],[[100,idio_beta,0,ind_beta],
                                                                                 [100,idio_beta,0,ind_beta]],
    ...
    if 'so_monop_duop' in model_list:
        #MONOP_DUOP PARAM LIST, SOPHISTICATED:
        #FIRM 1, THE DUOPOLIST, IS SOPHISTICATED.
        param_list.append([2, 'q','so', 'monop_duop',[100,idio_beta,0,ind_beta],[[100,idio_beta,0,ind_beta],
                                                                                 [100,idio_beta,0,ind_beta]],
                           [[1,1,0,1],[1,1,0,1]],phlen,[1,phx_var,1,x_var,2,phq_var],
                           50,periods,periods,max_q,mult_cap,iterations,
...
'''
INTERCEPT MODEL: PARAMETERS and PARAM_LIST CREATION
- building_1: getting the model up and running, but the calibration on the initial history wasn't working
    - _2: changed what is stored in "values" lists from x1*Q, x2*Q, x3*Q to x1, x2, Q. --> Still has the same issue
    - _3: adding in the Q/p graphs for the prehistory.
        - Each iteration looks good, approx. -20 slope for each iteration...
          so this points to something potentially wrong with the calibration.
    - _4: changed the Q function inside of chooseQ
    - _5: fixed the price equation in gen_init_hist
    - _6: made more changes to the price eqn in gen_init_hist by returning it to original formulation but changing values list.
        - When idio_alpha = 0, alpha_0 = 100, and ind_beta = -15, the graphs show all three of these things being zero.
'''
param_list = []
...
inc_idio_alpha = -5
ind_beta = -15 #standard ind_beta = -15
phx_var = 0.5 #SOMETIMES CHANGED TO REFLECT EARLIER PAPER. IN THIS VERSION PHX_VAR = 0.5. IN OLD VERSION, PHX_VAR = 0.3
phq_var = 0.5
x_var = 0.3
...
''' General:
    Iterates over the set of parameters (created above).
    run_simulation returns the name of the csv containing the run information.
    graph_sns uses csv_name returned by run_sim'n to pull run info from the csv and graph in-run variables, ex. Q, P, etc.
    Prints number of parameter currently running and shows when all runs are finished.

    Version-specific (trickperiod*):
    csv_name is a data frame containing a name for each (trick amount, trick period),
    instead of just a name per run (which requires graphing to be done right afterwards so name is not overwritten;
    graphing of run still happens in this for-loop, but it is not necessarily done like this).
    Necessary to initiate a csv_name dictionary before the for-loop.

    Comment this section out for sweep
    '''
...
param_iter = 1
csv_name_dict = dict() #saving all csv names to dictionary.

iw = {}
for param in param_list:
    #start = timeit.default_timer()
    print('beginning param iteration ' + str(param_iter) + ' of ' + str(len(param_list)))
    output_file.write('\n beginning param iteration ' + str(param_iter) + ' of ' + str(len(param_list)))
    #param_time_est = 0.0122 * param[10] * param[13]


    #Runs the simulation
    csv_name, initial_weights = run_simulation(*param) #returns name of csv where all simulation data is saved
    print(csv_name)
    iw[param_iter] = initial_weights
    print(initial_weights)
    csv_name_dict[param_iter] = csv_name
    #Graphs for each parameter run
    graph_sns(csv_name, initial_weights, *param)

    print('param iteration ' + str(param_iter) + ' complete')
    output_file.write('\n param iteration ' + str(param_iter) + ' complete')
...
graph_soph_unsoph: produces an overlap graph of the both correct and both incorrect parameterizations.
- Takes csv_name, initial_weights, and the parameters for the sr_base_duop and so_base_duop models.
- Takes the entire param_list as an input and sorts through it to find the right models within the function.
'''

#no longer exists in this version, has been generalized: graph_correct_incorrect(csv_name_dict, initial_weights, param_list)

##Comparing both_correct to unsophisticated base duopoly:
#two_graph_overlap(0, 1, csv_name_dict, iw, param_list, model_list)
#three_graph_overlap(0, 1, 2, csv_name_dict, iw, param_list, model_list)
##Comparing both_correct to sophisticated base duopoly:
#two_graph_overlap(0, 2, csv_name_dict, initial_weights, param_list, model_list)

'''
Creates sweep_param_list.
- This block of code creates the parameter list for a beta sweep.
- For the industry model, the sweep types are:
    - alpha_sweep_fixed: sweeps over the alpha_0 param while keeping everything else the same.
    - idio_sweep_fixed: sweeps over the idio_alpha param while keeping everything else the same.
'''


sweep_param_list = []

sweep_type = 'idio_sweep_fixed' 
#sweep types: 'idio_sweep_fixed', 'b' = 'beta_sweep_fixed', 'int_sweep_fixed', 'phlen_sweep'

last_fold = str(sweep_type) + '_srmonop_it' + str(iterations) + '_captest(-15, -10, -5, -2.5)'
#last_fold = str(iterations) + 'it_' + str(sweep_type) + '_int' + str(alpha_0) + '_beta' + str(ind_beta) + '_correcta' + str(idio_alpha) + '_11'
sweep_directory = os.path.join(os.getcwd(), 'MY_v' + str(version_no),
            last_fold)
...
if sweep_type == 'idio_sweep_fixed':
    #This sweeps over the true idiosyncratic beta. Both firm 1 and firm 2 have the same idiosyncratic beta magnitude (the true beta), although they still have different mental models.
    option_list = (-15, -10, -5, -2.5)
elif sweep_type == 'idio_inc_sweep':
    option_list = (-10, -5, -2.5, +2.5)
elif sweep_type == 'b':
    option_list = (-5, -10, -15, -20)
elif sweep_type == 'int_sweep_fixed':
    option_list = (90, 95, 100, 105, 110)
elif sweep_type == 'phlen_sweep':
    option_list = (20, 100, 1000)
elif sweep_type == 'entrepreneur_idio_alpha':
    option_list = (-10, -5, -2)
else:
    print('invalid sweep type')
#########BUILDING HERE NOW: Other sweep types############
#if sweep_type == 'beta_sweep_fixed':
    #beta_option_list = ()



model_list = ['sr_monop_duop']
##all models: ['both_correct', 'both_incorrect', 'sr_base_duop', 'so_base_duop', 'sr_monop_duop', 'so_monop_duop']
...
for option in option_list:
    ...
    if sweep_type == 'idio_sweep_fixed':
        idio_alpha = option
        inc_idio_alpha = option
        inc_alpha_0 = alpha_0
        inc_ind_beta = ind_beta
        ent_alpha_0, ent_idio_alpha, ent_ind_beta = alpha_0, idio_alpha, ind_beta  #the duopolist is correct
        #ind_beta already set above
    elif sweep_type == 'idio_inc_sweep':
        inc_idio_alpha = option #only the incorrect firm's idio alpha is being swept over
        inc_alpha_0 = alpha_0
        inc_ind_beta = ind_beta
        ent_alpha_0, ent_idio_alpha, ent_ind_beta = alpha_0, idio_alpha, ind_beta
    elif sweep_type == 'b':
        ind_beta = option
        inc_ind_beta = option
        inc_idio_alpha = idio_alpha
        inc_alpha_0 = alpha_0
        ent_alpha_0, ent_idio_alpha, ent_ind_beta = alpha_0, idio_alpha, ind_beta
    elif sweep_type == 'int_sweep_fixed':
        alpha_0 = option
        ...
        ##In this case, the entrepreneur/'correct' firm's parameters can also differ from true demand.
        ent_idio_alpha = option
        #idio_alpha and inc_idio_alpha are already set
        ent_alpha_0 = alpha_0
        inc_alpha_0 = alpha_0
        ent_ind_beta = ind_beta
        inc_ind_beta = ind_beta
    else:
        print('invalid sweep type')
        
        
    if 'both_correct' in model_list:
        #0. BOTH CORRECT LIST:
        sweep_param_list.append([2, 'q','sr', 'base_duop',[alpha_0,idio_alpha,0,ind_beta],[[alpha_0,idio_alpha,0,ind_beta],
                                                                       [alpha_0,idio_alpha,0,ind_beta]],
                      [[1,1,0,1],[1,1,0,1]],phlen,[1,phx_var,1,x_var,2,phq_var],
                       50,periods,periods,max_q,mult_cap,iterations,
                       trick_params, 0, 0, #trick_params, monop_firm, so_firm
                       'six_per_page', os.path.join(sweep_directory, 'cm=both_correct') ,'base_sr_',1])
    
    if 'both_incorrect' in model_list:
        #1. BOTH INCORRECT LIST:
        sweep_param_list.append([2, 'q','sr', 'base_duop',[alpha_0,idio_alpha,0,ind_beta],[[alpha_0,0,inc_idio_alpha,inc_ind_beta],
                                                                       [alpha_0,0,inc_idio_alpha,inc_ind_beta]],
                      [[1,0,1,1],[1,0,1,1]],phlen,[1,phx_var,1,x_var,2,phq_var],
                       50,periods,periods,max_q,mult_cap,iterations,
                       trick_params, 0, 0, #trick_params, monop_firm, so_firm
                       'six_per_page', os.path.join(sweep_directory, 'cm=both_incorrect') ,'base_sr_',1])
        
    
    if 'sr_base_duop' in model_list:
        #BASE DUOP PARAM LIST, SELF-REFLECTIVE:
        sweep_param_list.append([2, 'q','sr', 'base_duop',[alpha_0,idio_alpha,0,ind_beta],[[ent_alpha_0,ent_idio_alpha,0,ent_ind_beta],
                                                                   [inc_alpha_0,0,inc_idio_alpha,inc_ind_beta]],
                  [[1,1,0,1],[1,0,1,1]],phlen,[1,phx_var,1,x_var,2,phq_var],
                   50,periods,periods,max_q,mult_cap,iterations,
                   trick_params, 0, 0, #trick_params, monop_firm, so_firm
                   'six_per_page', os.path.join(sweep_directory, 'cm=sr_base_duop') ,'base_sr_',1])    
           
    
    if 'so_base_duop' in model_list:
        #BASE DUOP PARAM LIST, SOPHISTICATED:
        #FIRM 1 IS SOPHISTICATED.
        sweep_param_list.append([2, 'q','so', 'base_duop',[alpha_0,idio_alpha,0,ind_beta],[[ent_alpha_0,ent_idio_alpha,0,ent_ind_beta],
                                                                             [inc_alpha_0,0,inc_idio_alpha,inc_ind_beta]],
                          [[1,1,0,1],[1,0,1,1]],phlen,[1,phx_var,1,x_var,2,phq_var],
                          50,periods,periods,max_q,mult_cap,iterations,
                          trick_params, 1, 0, #trick_params, monop_firm, so_firm
                          'six_per_page', os.path.join(sweep_directory, 'cm=so_base_duop'),'base_so',2])

    
    
    if 'sr_monop_duop' in model_list:
        #MONOP_DUO PARAM LIST, SELF-REFLECTIVE:
        sweep_param_list.append([2, 'q','sr', 'monop_duop',[alpha_0,idio_alpha,0,ind_beta],[[ent_alpha_0,ent_idio_alpha,0,ent_ind_beta],
                                                                                 [alpha_0,inc_idio_alpha,0,inc_ind_beta]],
                          [[1,1,0,1],[1,1,0,1]],phlen,[1,phx_var,1,x_var,2,phq_var],
                           50,periods,periods,max_q,mult_cap,iterations,
                           trick_params, 1, 0, #trick_params, so_firm
                           'six_per_page', os.path.join(sweep_directory, 'cm=sr_monop_duop'),'monop_sr_',3])
    
    if 'so_monop_duop' in model_list:
        #MONOP_DUOP PARAM LIST, SOPHISTICATED:
        #FIRM 1, THE DUOPOLIST, IS SOPHISTICATED.
        sweep_param_list.append([2, 'q','so', 'monop_duop',[alpha_0,idio_alpha,0,ind_beta],[[ent_alpha_0,ent_idio_alpha,0,ent_ind_beta],
                                                                                 [alpha_0,inc_idio_alpha,0,inc_ind_beta]],
                           [[1,1,0,1],[1,1,0,1]],phlen,[1,phx_var,1,x_var,2,phq_var],
                           50,periods,periods,max_q,mult_cap,iterations,
                           trick_params, 1, 0,#for 'so': trick_params, monop_firm, so_firm
                           'six_per_page', os.path.join(sweep_directory, 'cm=so_monop_duop'),'monop_so_',4])
...
for param in sweep_param_list:
    for model in model_list:
        start = timeit.default_timer()
        print('beginning param iteration ' + str(sweep_param_iter) + ' of ' + str(len(sweep_param_list) * len(model_list)))

        if not os.path.exists(sweep_directory):
            os.makedirs(sweep_directory)
        output_file = open(os.path.join(sweep_directory,'sweep_output_file.txt'), 'w+')
        output_file.write('\n beginning param iteration ' + str(sweep_param_iter) + ' of ' + str(len(sweep_param_list)))

        #CHANGE IN PROGRESS: Always uses incorrect person's idio_beta as index_beta to store csv, since this always changes
        if param[3] == 'base_duop':
            if sweep_type == 'idio_alpha_fixed':
                index_param = param[4][1] #correct firm's idio_alpha
            elif sweep_type == 'idio_sweep_fixed':
                index_param = param[4][1] #correct firm's idio_alpha
            elif sweep_type == 'idio_inc_sweep':
                index_param = param[5][1][2]
            elif sweep_type == 'b':
                index_param = param[4][3] #correct model's industry beta
            elif sweep_type == 'int_sweep_fixed':
                index_param = param[4][0] #correct model's intercept
            elif sweep_type == 'phlen_sweep':
                index_param = param[7]
            else:
                index_param = param[5][1][2] #incorrect firm's idio_alpha
        else:
             if sweep_type == 'entrepreneur_idio_alpha':
                index_param = param[5][0][1]
                print(index_param)
                print(param[5])
             else:
                index_param = param[5][1][1]
        
        #model = str(param[2]) + '_' + str(param[3])

        print(index_param, "is the param being swept over.")
        print(model, "is the model")
        print(param)
        
        print(csv_name)

        #Without parallel processing: about 1 min per run at 100 iterations; almost 5 minutes at 500 iterations.
        #With parallel processing of iterations, not in the grid (so 8 processors): about 30 seconds per run at 100 iterations, about 3 min at 500 iterations
        csv_name[index_param, model], initial_weights[index_param, model] = run_simulation(*param)
        print(csv_name)
        graph_sns(csv_name[index_param, model], initial_weights[index_param, model], *param)
        dir_list[model] = [os.path.join(os.getcwd(), 'MY_v' + str(version_no),
                last_fold, 'cm='+ model), [*param]]

        print('param iteration ' + str(sweep_param_iter) + ' complete')

        output_file.write("Done this param's run.")
        stop = timeit.default_timer()
        print('Time: ', stop - start)
        output_file.write('\n Time: ' + str(stop - start))
        sweep_param_iter += 1
...
'''
Graphing the sweep dataframes.
'''

df_dict = dict()
index_beta_list = list()

for param in sweep_param_list:
    for model in model_list:
        if param[3] == 'base_duop':
            if sweep_type == 'idio_alpha_fixed' or sweep_type == 'idio_sweep_fixed':
                index_param = param[4][1] #correct model's idio_alpha
                print('index param inside the loop:' + str(index_param))
            elif sweep_type == 'idio_inc_sweep':
                index_param = param[5][1][2]
            elif sweep_type == 'b':
                index_param = param[4][3] #correct model's industry beta
            ...
            if sweep_type == 'entrepreneur_idio_alpha':
                ...
                index_param = param[5][1][1]
        print('index param:' + str(index_param))
        name = csv_name[index_param, model]
        df_dict[index_param, model] = pd.read_csv(os.path.join(dir_list[model][0], name))
    if sweep_type == 'true_balanced' or sweep_type == 'incorrect_balanced':
        if index_param not in index_beta_list:
            index_beta_list.append(index_param)
            print('index beta list:' + str(index_beta_list))
'''
if sweep_type == 'idio_alpha_fixed' or sweep_type == 'incorrect_balanced':
    graph_sns_overlap(df_dict, index_beta_list, model_list, sweep_directory, dir_list, initial_weights, sweep_type)
else:
    graph_sns_overlap(df_dict, beta_option_list, model_list, sweep_directory, dir_list, initial_weights, sweep_type)
'''
graph_sns_overlap(df_dict, option_list, model_list, sweep_directory, dir_list, initial_weights, sweep_type)
...
'''
This block creates the parameter list for the tricks

Updates v2_17: adding a custom trick type for analysis purposes; trying to figure out if an absolute advantage through manipulation is possible.
'''


'''
#trick types: 'constQ', 'constQ_spikes', 'none'
trick_type = 'custom_percent'

model_list = ['somonop_duop'] #, 'somonop_duop'; 'sobase_duop',
...
#Note: trick amount = 1000 indicates no trick.

if trick_type == 'none':
    tamount_space = [1000]
    tperiod_space = []
elif trick_type == 'constQ':
    #Since trick is constant, Q = trick amount during trick periods
    tamount_space = [1000, 0.8, 0.9, 1.1, 1.15, 1.2]
    #If no spikes, then the tperiod is all periods of the simulations
    tperiod_space = [0, 1, 2, 3, 4]
    #all periods: 
    #tperiod_space = range(periods)
elif trick_type == 'constQ_spike':
    #Since trick is constant, Q = trick amount during trick periods:
    tamount_space = [1000, 1, 1.5, 2, 2.5, 3]
    #If there are spikes, then the tperiods need to be selected:
    tperiod_space, tperiod_ranges = [],[]
    i = 0
    while i < periods:
        l = [n for n in range(i, i+trick_len)]
        tperiod_ranges.append(l)
        i += (trick_len + trick_freq)
    tperiod_space = list(itertools.chain(*tperiod_ranges))
elif trick_type == 'custom':
    tamount_space = [5, 1000]
    tperiod_space = [0, 1]
elif trick_type == 'custom_percent':
    tamount_space = [1.1, 1.4, 1.5, 1.6, 1000]
    tperiod_space = range(10)
elif trick_type == 'old_paper_test': 
    tamount_space = [1.2]
    tperiod_space = range(19)
    
last_fold = 'trick_' + trick_type + '_' + str(iterations) + 'it_6_24' #+ str(tamount_space) + '_tp_' + str(tperiod_space)
directory = os.path.join(os.getcwd(), 'MY_v' + str(version_no),
            last_fold, str([100,idio_beta,0,ind_beta]))
'''

''' Create a set of parameters for the simulation at each trick amount. The specific trick amount is included
    in the trick_params.

    Note that a difference between this version and other masters is that all trick-related parameters are
    consolidated into one list of trick-related params.
'''
...
for ta in tamount_space:
    ...
    #BASE DUP PARAM LIST, SOPHISTICATED:
    #FIRM 1 IS SOPHISTICATED
    param_list.append([2, 'q','so', 'base_duop',[100,idio_beta,0,ind_beta],[[100,idio_beta,0,ind_beta],                                                                               [100,0,idio_beta,ind_beta]],
                          [[1,1,0,1],[1,0,1,1]],phlen,[1,phx_var,1,x_var,2,phq_var],
                          50,periods,periods,max_q,mult_cap,iterations,
                          trick_params, 1, 0, #trick_params, monop_firm, so_firm
                           'six_per_page', os.path.join(directory, 'cm=sobase_duop'),'base_so_',2])
    ...
    param_list.append([2, 'q','so', 'monop_duop',[100,idio_beta,0,ind_beta],[[100,idio_beta,0,ind_beta],
                        ...
                        50,periods,periods,max_q,mult_cap,iterations,
                        trick_params, 1, 0, #trick_params, monop_firm, so_firm
                        'six_per_page', os.path.join(directory, 'cm=somonop_duop'),'monop_so_',4])
    
'''

'''
#This block runs the simulation for tricks.
...
if not os.path.exists(directory):
        os.makedirs(directory)
output_file = open(os.path.join(directory,'trick_output_file.txt'), 'w+')
for param in param_list:
    start = timeit.default_timer()
    print('beginning param iteration ' + str(param_iter) + ' of ' + str(len(param_list)))
    ...
    ##PARAM[16] =/= trick amount bc changed to trick_params!
    ta = param[15][1]
    model = str(param[2]) + str(param[3])

    csv_name[ta, model], initial_weights[ta, model] = run_simulation(*param) #returns name of csv where all simulation data is saved
    dir_list[model] = [os.path.join(directory, 'cm=' + model), [*param]]
    #graph_sns(csv_name[ta, model], initial_weights, *param)

    #print(dir_list[model])
    #print(dir_list[model][1])

    output_file.write('param iteration ' + str(param_iter) + ' complete')
    print('param iteration ' + str(param_iter) + ' complete')
    stop = timeit.default_timer()
    print('Time: ', stop - start)
    output_file.write('\n Time: ' + str(stop - start))
...
#This block runs the graphs for tricks.
...
for model in model_list:
    for ta in tamount_space:
        #print(model)
        name = csv_name[ta, model]
        df_dict[ta, model] = pd.read_csv(os.path.join(dir_list[model][0], name))


#if trick_type != 'none':
graph_trick_overlap(df_dict, tamount_space, model_list, directory, dir_list, initial_weights)
#master_df = profcomp_graph(df_dict, tamount_space, model_list, directory)
#cumprofcomp_graph(df_dict, tamount_space, model_list, directory)
#absprof_graph(df_dict, tamount_space, model_list, directory, 0) #0 = firmnum (indicated firm 1, the sophisticated firm)
#sophfirm_cumabsprof(df_dict, tamount_space, model_list, directory)
